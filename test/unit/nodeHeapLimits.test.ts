...
describe("nodeHeapLimits", () => {
  ...
  it("parses max old space size from NODE_OPTIONS", () => {
    ...
    expect(parseMaxOldSpaceSizeMb()).toBe(768);
  });

  it("derives warn and ceil bytes from heap cap", () => {
    const limits = memoryLimitsFromNodeHeap(768);
    expect(limits.warnBytes).toBe(Math.floor(768 * 0.72) * 1024 * 1024);
    expect(limits.ceilBytes).toBe(Math.floor(768 * 0.9) * 1024 * 1024);
    expect(limits.rssWarnBytes).toBe(600 * 1024 * 1024);
  });
});
